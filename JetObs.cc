...
double hadspec[32][2], hadspectwo[32][2];
...
void jet_obs (vector<Parton> partons, vector<Quench> quenched, vector<Hadron> vhadrons, vector<Hadron> qhadrons, std::string part_or_had, std::string cent, int tmethod, double alpha, double R, std::string system, double weight, double cross, std::string pdf)
{
	...
	ClusterSequence csq(particlesq, jet_def);
        vector<PseudoJet> jetsq = sorted_by_pt(csq.inclusive_jets(0.));
	...
	*/
	...
	//odir << part_or_had.c_str() << "_" << pdf.c_str() << "_tmethod_" << tmethod << "_alpha_" << int(100.*alpha);
	odir << "./results/observables/" << part_or_had.c_str() << "_";
	ostringstream pRaaF, RaaF, AcoF;
	
	double normi=(crosssum/double(nev+1))/weightsum;
	...
	for (unsigned a=0; a<32; a++) {
		...
		if (hadspec[a][0]!=0.) praaerr=sqrt(pow(sqrt(hadspectwo[a][1])/hadspec[a][0],2.)+pow(hadspec[a][1]/pow(hadspec[a][0],2.)*sqrt(hadspectwo[a][0]),2.));
    		pRAA << (bincut[a]+bincut[a+1])/2. << " " << hadspec[a][0]*normi << " " << sqrt(hadspectwo[a][0])*normi << " " << hadspec[a][1]*normi << " " << sqrt(hadspectwo[a][1])*normi << " ";
	...
	for (unsigned int i=0; i<100; i++)
	{
		...
		RAA << ptbin*double(i)+ptbin/2. << " " << jet_spec_hist[nr][0][i]*normi << " " << sqrt(jet_spec_histtwo[nr][0][i])*normi << " "
		<< jet_spec_hist[nr][1][i]*normi << " " << sqrt(jet_spec_histtwo[nr][1][i])*normi << " ";
