...
int main(int argc, char** argv)
{
	...
	//Partons Output File
	char outPart[100];
	sprintf(outPart,"../results/%s/simple_T%iA%iPARTONS.dat",cent.c_str(),tmethod,int(alpha*100.));
	ofstream part_file;
	...
	//Wake Output File
        char outWake[100];
        sprintf(outWake,"../results/%s/simple_WAKE.dat",cent.c_str());
        ofstream wake_file;
	...
	do {
		...
		//Force to do 10 jobs per hydro - for mayank sources
		Nhyd=1;
		...
		do {
			...
			ofstream source_file;
	        	sprintf(Sfile,"../results/%s/sources/hyd%i/simpleSOURCE%i.dat",cent.c_str(),totcount+1,count);
			...
			partons.clear();
			vector<double> sp;
			sp.push_back(200.), sp.push_back(0.), sp.push_back(0.), sp.push_back(200.);
			partons.push_back ( Parton ( sp, 0., 0., -1, -1, -1, 21, "ps", 0, 0, true ) );
