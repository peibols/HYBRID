...
double deltax=0.1973;
double deltay=0.1973;
double deltaeta=0.203125;
double deltat=0.2;

double tau0=0.4;	//Initial time
...
void read_hydro(int nhyd, std::string cent)
{
	...
	/*
	double step=0.01;
	cout << " T at center= " << hydrot[0][32][75][75] << endl;
	for (unsigned int i=0; i<1000; i++)
	{
		//double x=0.8*double(i)*step;
		double temp=gT(0.3+double(i)*step,0.,0.,0.);
		cout << 0.3+double(i)*step << " " << temp << endl;
		if (temp==0.) break;
	}
	*/
}
