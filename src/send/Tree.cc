...
void TreeDoer(int nhat, int seed, vector<Parton>& partons);

//Need to set precision higher so that no velocities = 1 (unless only moving in one direction and light-like)

//Args: nhat, seed, partons
void TreeDoer(int nhat, int seed, vector<Parton>& partons)
{
	Pythia pythia;
