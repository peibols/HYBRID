...
//Args: nhat, N, seed, sqrts 
int main(int argc, char** argv) {

  assert(argc==5);

  std::string sqrts=argv[4];
  std::string tpdf="MED";

  int nEvent  = atoi(argv[2]);
  int core = atoi(argv[3]); //Not used

  double ptmin=0.;
  double ptbin=2.;
  double jetspec[500][4];
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
	...
	if (npt!=-1000 && fabs(raper)<=0.35) {
	  if (sqrts=="020" && int(ide)==111) hadspec[npt]+=weight;
	  if (sqrts!="020" && charge!=0.) hadspec[npt]+=weight;
	}
      }
    }
  ...
  for (int rad=2; rad<=5; rad++) {
    ...
    fg << "../../results/" << tpdf << "_pthat" << nhatmin << "_N" << nEvent << "_JETR0" << rad << "spec" << core << ".dat";
  ...
  std::ostringstream fg;
  fg << "../../results/" << tpdf << "_pthat" << nhatmin << "_N" << nEvent << "_HADRONspec" << core << ".dat";
  std::ofstream outfile5(fg.str().c_str(),std::ios_base::binary);
  for (unsigned a=0; a<31; a++) {
