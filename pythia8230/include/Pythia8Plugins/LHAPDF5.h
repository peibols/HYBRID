...
extern "C" {
  ...
  //Nuclear evolution
  extern void evolvepdfam_(int&, double&, double&, double&, double*); 
...
namespace LHAPDF5Interface {
  ...
  // Evaluate x f_i(x, Q) for the nuclear case.
  void evolvePDFaM( int& nSet, double x, double Q, double a, double* xfArray) {
    evolvepdfam_( nSet, x, Q, a, xfArray);
  }
...
public:
  ...
  LHAPDF5(int idBeamIn, string setName, int member,  int nSetIn = -1,
    ...
    isPb = (idBeamIn == 2210) ? true : false;
    isAu = (idBeamIn == 2211) ? true : false; 
    if (isPb==true) cout << " idBeamIn= " << idBeamIn << " is a Pb";
    else if (isAu==true) cout << " idBeamIn= " << idBeamIn << " is a Au";
    else cout << " idBeamIn= " << idBeamIn << " is a proton likely";}
...
void LHAPDF5::xfUpdate(int, double x, double Q2) {
  ...
  //Nuclear effects for Pb
  else if (isPb) {
    LHAPDF5Interface::setPDFparm( "EPS09LO" );
    LHAPDF5Interface::evolvePDFaM( nSet, x, Q, 208, xfArray);
    xPhoton=0.0;
  }

  //Nuclear effects for Au
  else if (isAu) {
    LHAPDF5Interface::setPDFparm( "EPS09LO" );
    LHAPDF5Interface::evolvePDFaM( nSet, x, Q, 197, xfArray);
    xPhoton=0.0;
  }
