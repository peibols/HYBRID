...
const double Pythia::VERSIONNUMBERCODE = 8.230;
...
bool Pythia::init() {
  ...
  if (!doProcessLevel) boostType = 1;
    ...
    if ( beamA.hasResGamma() || beamB.hasResGamma() ) {
      if ( doDiffraction || doHardDiff ) {
        info.errorMsg("Abort from Pythia::init: "
          "Diffractive events not implemented for photon-photon collisions");
        return false;
...
bool Pythia::checkBeams() {
  ...
  if (isLeptonA && isLeptonB ) {
    ...
    if (beamHasGamma) {
      ...
      // No diffraction with photons.
      } else if ( doDiffraction || doHardDiff ) {
        info.errorMsg("Error from Pythia::init: "
          "Diffractive events not implemented for photon-photon collisions");
        return false;
      }
  ...
  bool isHadronA = (idAabs == 2212) || (idAabs == 2112) || (idA == 111)
                || (idAabs == 211)  || (idA == 990) || (idAabs == 2210) || (idAabs == 2211);
  bool isHadronB = (idBabs == 2212) || (idBabs == 2112) || (idB == 111)
                || (idBabs == 211)  || (idB == 990) || (idBabs == 2210) || (idBabs == 2211);
...
PDF* Pythia::getPDFPtr(int idIn, int sequence, string beam, bool resolved) {
  ...
  if (abs(idIn) == 2212 || abs(idIn) == 2112 || abs(idIn) == 2210 || abs(idIn) == 2211) {
