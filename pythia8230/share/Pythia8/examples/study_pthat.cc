...
int main(int argc, char** argv) {
  ...
  */
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    //Print Pthat
    cout << " Pthat of event= " << info.pTHat() << endl;
    for (int i=0; i<pythia.event.size(); i++) {
      //Print two hardest guys
      if (pythia.event[i].status()==-23) {
        cout << " particle= " << i << " has Px= " << pythia.event[i].px() << " " << " has Py= " << pythia.event[i].py() << " has Pz= " << pythia.event[i].pz() << " has E= " << pythia.event[i].e() << ", and Pt= " << pythia.event[i].pT() << endl;
      }
    ...
    for (int rad=3; rad<=3; rad++) {
	  ...
	  cout << " Jet#" << nj << " has Px= " << jets[nj].px() << " has Py= " << jets[nj].py() << " has Pz= " << jets[nj].pz() << " has E= " << jets[nj].e() << " has Pt= " << jets[nj].pt() << endl;
          if (jets[nj].pt()-30>info.pTHat()) {
	    cout << "\n AKIIII HUBO PERKHAAAAL \n \n";
            pythia.event.list();
          }
